                ...
                <Grid container spacing={2}>
                    {userDetails.map((item, index) => (
                        <Grid item xs={12} sm={6} md={4} lg={3} key={index}>
                            <ItemCardJoin title={item.eventName} venue={item.venue} date={new Date(item.date).toISOString().split('T')[0]} time={new Date(item.date).toISOString().split('T')[1].split('.')[0]} slots={item.slotsRemaining} />
                        </Grid>
                    ))}
                </Grid>
                <br></br>
